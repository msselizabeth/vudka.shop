...
const RodsList = ({ rods }) => {
  ...
  useEffect(() => {
    const uniqueTyperods = [
      ...new Set(
        rods
          .filter((rod) => rod.render)
          .map((rod) => rod.typerods)
  ...
  const filteredProducts = allRods
    .filter(
      (rod) =>
        ...
        (filters.brand.length === 0 || filters.brand.includes(rod.brand)) &&
        (filters.series.length === 0 || filters.series.includes(rod.series)) &&
        (filters.testMax.length === 0 ||
          filters.testMax.includes(rod.testMax)) &&
        (filters.testLb.length === 0 || filters.testLb.includes(rod.testLb)) &&
        (filters.action.length === 0 || filters.action.includes(rod.action)) &&
        (filters.section.length === 0 || filters.section.includes(rod.section))
    )
    .sort((a, b) => {
            ...
            if (product.render) {
              return (
                <li key={product._id} className={styles.rods__item}>
                  <p className={styles.rods__article}>
